...
(deftest create-torrent-handler-missing-path
  (with-redefs [session/create-session! (fn [_] (throw (ex-info "boom" {})))]
...
(deftest create-torrent-handler-success
  (let [fake-session {:id "S1" :name "N1"}
    ...
    (with-redefs [session/create-session! (fn [_] fake-session)
                  session/session->summary (fn [_] fake-summary)]
...
(deftest details-handler-not-found
  (with-redefs [session/get-session (fn [_] nil)]
...
(deftest pause-resume-stop-delete-handlers
  (let [sess {:id "S1"}
    ...
    (testing "pause not found"
      (with-redefs [session/pause! (fn [_] false)]
        (let [resp (api/pause-handler {:path-params {:id "X"}})]
          (is (= 404 (:status resp))))))

    (testing "pause ok"
      (with-redefs [session/pause! (fn [_] true)
                    session/get-session (fn [_] sess)
                    session/session->details (fn [_] details)]
        (let [resp (api/pause-handler {:path-params {:id "S1"}})]
          (is (= 200 (:status resp)))
          (is (= details (:body resp))))))

    (testing "delete ok"
      (with-redefs [session/delete! (fn [_] true)]
